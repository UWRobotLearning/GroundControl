...
def main():
    """Collect with RSL-RL agent from multiple checkpoints."""
    # parse configuration
    ...
    print(f"[INFO] Loading experiments from directory: {log_root_path}")
    
    # Get all checkpoint paths
    checkpoint_pattern = os.path.join(log_root_path, agent_cfg.load_run, "model_*.pt")
    ...
    log_dir = os.path.dirname(checkpoint_paths[0])
    ...
    total_dataset_size = int(1e6)  # Total desired buffer size
    replay_buffer = ReplayBuffer(observation_space, action_space, capacity=total_dataset_size, next_observation_space=observation_space)
    ...
    # Calculate data to collect per checkpoint
    data_per_checkpoint = total_dataset_size // len(checkpoint_paths)
    ...
    for checkpoint_idx, checkpoint_path in enumerate(checkpoint_paths):
        ...
        # collect data for this checkpoint
        while len(replay_buffer) < checkpoint_idx*data_per_checkpoint + data_per_checkpoint:
            ...
            with torch.inference_mode():
                ...
                print(f"Checkpoint: {os.path.basename(checkpoint_path)}, Replay buffer size: {len(replay_buffer)}")

                obs = new_obs

            if args_cli.video:
                # ... (video recording logic, if needed)
                pass

    replay_buffer_path = os.path.abspath("./all_checkpoints_ppo_buffer_simple_reward.npz")
...
python source/standalone/workflows/rsl_rl/collect_all_checkpoints.py --task Isaac-Velocity-Flat-Simple-Rewards-Unitree-A1-v0 --num_envs 64 --load_run 2024-09-26_15-07-25
'''